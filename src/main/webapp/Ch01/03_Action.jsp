...
<body>
	...
	<!-- 
		-form태그내의 submit 버튼을 누르면 form에 입력한 입력사항들이 name과 value로 쌍을 이뤄서
		 Action의 지정한 곳에 전송된다. Action 지정되는 곳은 서버내의 특정 페이지 또는 서블릿들이 지정될 수 있다.
		 -Action : 입력된 데이터들이 전송되어질 목적지
		 -method : get를 지정하면url속에 데이터가 노출되고, post를 지정하면 url속에 데이터가 나타나지않는다. 따로쓰지않으면 get
	 -->
