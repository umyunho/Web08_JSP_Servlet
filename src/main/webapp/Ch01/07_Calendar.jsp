...
.container{ width:560px; margin:0 auto; border:1px solid gray; display:flex; justify-content: conter; flex-direction:column;
...
<body>
	<%
	Calendar sDay = java.util.Calendar.getInstance();	//오늘날짜 객체 생성
	int sYear = sDay.get(Calendar.YEAR);	//년도 추출
	int sMonth = sDay.get(Calendar.MONTH);	//월 추출
	
	// 최초실행시 request.getParameter("sYear")는 null
	//이전달이나 다음달 버튼을 클릭하고 이동해온 경우는 null도 아니고 그 값으로 sYear와 sMonth를 교체해줘야함
	...
	sDay.set( sYear,sMonth, 1); //sDay객체를 현재달 1일로 수정
